		...
		<c:forEach var="category" items="${categories}">
			<c:choose>
				<c:when test="${category.category_depth == 1}">
					...
					<a class="ml-5 mr-3 h5 text-dark "
						href="/products/${category.category_code }">전체보기</a>
				</c:when>
				<c:otherwise>
					<a class="mr-3 h5 text-dark"
						href="/products/${category.category_code}">${category.category_name}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</div>
		...
		<c:forEach var="product" items="${products}">
			<c:if test="${product.on_sale == 1}">
				...
				<div class="col-sm-4 mb-5">
					...
					<span class="d-block text-dark h3 mb-3">[<c:out
