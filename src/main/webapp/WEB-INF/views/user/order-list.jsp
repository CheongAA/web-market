								...
								<div class="row">
									<div class="col-sm-8 p-3 m-2">
										<c:forEach var="orderDetail" items="${order.orderDetail}">
											<div class="row m-3">
												<img alt="" src="${orderDetail.product.product_img}" class="col-3 w-100 h-75 mr-3">
												<div class="col-8">
													<h5>
													...
													<c:if test="${order.order_state_id == 3 or order.order_state_id == 9}">
														<c:choose>
															<c:when test="${orderDetail.review_id == 0 }">
																<a class="btn btn-sm btn-outline-secondary" href="${pageContext.request.contextPath}/user/review/add/${orderDetail.order_detail_id}">후기쓰기</a>
															</c:when>
															<c:otherwise>
																<a class="btn btn-sm btn-outline-secondary" href="${pageContext.request.contextPath}/user/review">내가 쓴 후기</a>
															</c:otherwise>
														</c:choose>
													</c:if>
												</div>
									...
									<div class="col-sm-3 text-center my-auto p-2">
										...
										<a class="btn btn-sm btn-outline-primary btn-block" href="${pageContext.request.contextPath}/user/order/${order.order_id}">주문조회</a>
										<c:if test="${order.order_state_id == 3 or order.order_state_id == 9}">
