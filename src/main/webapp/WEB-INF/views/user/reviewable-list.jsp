...
<body>
	<div class="row">
		...
		<div class="col py-5 ml-5">
			<h3>구매후기</h3>
			<div class="row mt-5">
				<a class="border-top border-left border-right bg-white h5 p-3 m-0 text-dark text-decoration-none col-6" href="${pageContext.request.contextPath}/user/reviewable">작성 가능한
					구매목록</a>
				<a class="border bg-light h5 p-3 m-0 text-dark text-decoration-none col-6" href="${pageContext.request.contextPath}/user/review">내가 쓴 구매후기</a>
			</div>
					...
					<c:otherwise>
						<div class="col-sm-12 p-2 my-2">
							<c:forEach var="order" items="${orders}">
								<c:if test="${order.order_state_id ==3 or order.order_state_id ==9}">
										...
										<c:if test="${orderDetail.review_id == 0 }">
											<div class="row border-bottom">
												...
												<div class="col-sm-3 text-center my-auto p-2">
													<a class="btn btn-sm btn-outline-primary btn-block" href="${pageContext.request.contextPath}/user/review/add/${orderDetail.order_detail_id}">후기쓰기</a>
												</div>
											</div>
										</c:if>
									</c:forEach>
