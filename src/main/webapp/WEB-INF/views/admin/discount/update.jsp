...
<body>
	...
	<div class="row mt-5">
		...
		<div class="col-sm-12 p-5">
			<form action="${pageContext.request.contextPath}/admin/discount/update" method="post" enctype="multipart/form-data">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									<div class="mt-3">
										<input type="hidden" name="discount_thumbnailImg" value="${discount.discount_thumbnailImg}" />
										<img class="w-75 mb-2" src="${discount.discount_thumbnailImg}" id="origin_discount_img">
										<img class="w-75" id="select_discount_img">
									</div>
								</td>
							</tr>
							<c:set var="start">
								<fmt:formatDate value="${discount.discount_start}" pattern="yyyy-MM-dd" />
							</c:set>

							<c:set var="end">
								<fmt:formatDate value="${discount.discount_end}" pattern="yyyy-MM-dd" />
							</c:set>
							<tr>
								...
								<td>
									<select class="form-control" id="select_product_list" multiple>
										<c:forEach var="product" items="${products}">
											<c:if test="${product.discount_id == 0 or product.discount_id == discount.discount_id}">
							...
							<tr>
								...
								<td>
									<div id="select_product" class="d-block">
										<c:forEach var="product" items="${products}">
											<c:if test="${product.discount_id == discount.discount_id}">
