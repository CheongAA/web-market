					...
					<a href="${pageContext.request.contextPath}/product/${product.product_id}" class="text-decoration-none">
						...
						<c:choose>
							<c:when test="${product.product_quantity <= 0 }">
								<span class="d-block market-color h5 pl-3">품절</span>
							</c:when>
							<c:otherwise>
								<c:choose>
						...
						<span class="d-block text-dark font-weight-light mt-2 pl-3"> <c:out value="${product.product_desc}" />
						</span>
					</a>
				</div>
			</c:if>
