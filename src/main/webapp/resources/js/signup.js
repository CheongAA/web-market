...
function signup() {
	...
	if (idFlag && pwdFlag && nameFlag && emailFlag && phoneFlag && addressFlag) {
	...
	$.ajax({
		...
		success : function(data) {
			if (data == 1) {
				...
				$("#user_identification").removeClass('is-valid');
				$("#user_identification").addClass('is-invalid');
				signup();
			} else {
				if (!idE.test(inputValue)) {
					idFlag = false;
					$("#validation_id").text("4~12자의 영문 대소문자와 숫자로만 입력해주세요.");
					$("#validation_id").removeClass('text-primary');
					$("#validation_id").addClass('text-danger');
					...
					$("#validation_id").removeClass('text-danger');
					$("#validation_id").addClass('text-primary');

					$("#user_identification").removeClass('is-invalid');
					$("#user_identification").addClass('is-valid');
					signup();
				}
			}
		}
